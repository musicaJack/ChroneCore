...
- [x] NVS `clk_mode` 读写（默认数字；模拟为占位）
- [x] 1 Hz `chrone_ui_refresh` 秒 tick

---

## 阶段 3 — WiFi 与天气

- [ ] 引入 `78/esp-wifi-connect`
- [ ] `chrone_wifi`（C++）STA / AP 配网 / `force_ap`
- [ ] 配网页高级选项：城市列表 + NVS `weather.wx_city`
- [ ] 移植 `weather_city_list` + `weather_service`（C）
- [ ] Kconfig 心知 API Key
- [ ] 状态栏天气显示
