...
| 配置入口 | **长按表盘 3s** → **Settings** → **Alarms**（短振确认；见 [setup-hub-design.md](setup-hub-design.md)） |
...
## 4. 进入闹钟配置（修订：经 Settings Hub）

> **导航：** **长按 3s**（短振）→ **Settings** → **Alarms**；返回 **仅顶栏 Back** → Hub → 再 Back → 时钟。下列「左+右」为历史草案。
...
- 返回：列表/编辑 **Back → Settings Hub**；Hub **Back → 时钟**（不用底栏中键）。
